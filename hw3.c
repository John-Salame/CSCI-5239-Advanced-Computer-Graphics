...
float objX = 0;
float objY = 0;
const char* text[NUM_SHADERS] = {"Fixed Pipeline","Fireflies 1","Fireflies 2"};
int numPatches = 3;
int oldNumPatches = 0;
...
void display(GLFWwindow* window)
{
    ...
    */
    ...
    glUseProgram(shader[mode]);
    // do not produce light from fireflies on fixed pipeline
    glDisable(GL_LIGHTING);
    // pass the position of the fireflies at this current ModelViewMatrix to the shader;
    // the only way to do this in GL2 is by making them as point lights
...
int main(int argc,char* argv[])
{
   ...
   shader[2] = CreateShaderProg("firefly2.vert", NULL);
   //  Load textures
